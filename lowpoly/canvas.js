var lowPolyBg = (function() {
    ...
    getControlDots = function(element, x, y) {
        var ctrlDots = [],
        ...
        for (i = 0; i <= x; i++) {
            ...
            for (j = 0; j <= y; j++) {
                ctrlDots[i][j] = {
                    x: Math.floor(i * blockWidth),
                    y: Math.floor(j * blockHeight)
                };
            }
        }
    ...
    var getRandomDots = function(element, x, y) {
        ...
        for (i = 0; i < x; i++) {
            for (j = 0; j < y; j++) {
                dotX = Math.floor((Math.random() + i) * blockWidth);
    ...
    getDots = function(randomDots, a, b, step) {
    ...
    init = function() {
        ...
        if (cvs.getContext) {
            ...
            randomDots = getRandomDots(cvs, 4, 2);
            triangles = Delaunay.triangulate(randomDots);
            step = 0;
            draw = function() {
                var dots,
                    i,
                    ...
                    y3;
                step += 0.3;
                dots = getDots(randomDots, 20, 4, step);

                for (i = 0; i < triangles.length; i += 3) {
                    x1 = dots[triangles[i]][0];
